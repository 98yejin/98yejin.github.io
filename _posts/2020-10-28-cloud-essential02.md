...
> [참고]클라우드에서 workload는 클라우드에 배포 된 애플리케이션 또는 서비스를 의미한다. 이 서비스는 수백 개의 마이크로 서비스가 서로 연계되어 작동 하는 대규모 서비스이거나 개별 서비스일수 있다. 워크로드라는 용어는 추상화 및 이식성을 나타낸다. 서비스를 워크로드라고 부르는 것은 종속성이나 번거 로움없이 서로 다른 클라우드 플랫폼간에 또는 온 프레미스 에서 클라우드로 또는 그 반대로 이동할 수 있음을 의미한다. 클라우드의 컨테이너 기술은 작업을 중단하지 않고 워크로드를 이동할 수있는 훌륭한 도구이다. -[출처](https://www.8bitmen.com/a-super-helpful-guide-to-understanding-workload-its-types-in-cloud/)

Virtual Private Cloud; VPC를 사용하면 클라우드 환경 사용자가 가상 사설망(Virtual Private Network; VPN)을 정의하고, 조작하고, 제어하고, 해체하고, 재생성 한 다음 해당 정의(VPN)에 워크로드를 배포 할 수 있다. 
