...
: 클라우드 컴퓨팅을 사용하면 종량제 모델에서 온 디맨드로 원격 리소스를 활용하여 기술을 서비스로 활용할 수 있습니다. 클라우드에서 사용할 수있는 세 가지 주요 서비스 모델은 Infrastructure-as-a-Service, Platform-as-a-Service (PaaS) 및 Software-as-a-Service (SaaS)입니다.

-- IaaS provides the fundamental compute, network, and storage resources for customers on-demand.

: IaaS는 온 디맨드 고객에게 기본적인 컴퓨팅, 네트워크 및 스토리지 리소스를 제공합니다.
...
: PaaS는 고객이 생성했거나 타사에서 획득 한 애플리케이션을 개발, 배포, 관리 및 실행할 수있는 하드웨어, 소프트웨어 및 인프라를 고객에게 제공합니다.
...
: SaaS는 사용자에게 서비스 제공 업체의 클라우드 기반 소프트웨어에 대한 액세스를 제공합니다. 사용자는 클라우드에서 애플리케이션에 액세스하기 만하면 클라우드 제공 업체는 애플리케이션의 인프라, 플랫폼, 데이터, 애플리케이션 코드, 보안, 가용성 및 성능을 유지합니다.
...
: 배포 모델은 인프라가있는 위치, 인프라를 소유 및 관리하는 사람, 클라우드 리소스 및 서비스가 사용자에게 제공되는 방식을 나타냅니다. 클라우드에서 사용할 수있는 세 가지 주요 배포 모델은 퍼블릭, 프라이빗 및 하이브리드입니다.
...
: 퍼블릭 클라우드 모델에서 서비스 제공 업체는 데이터 센터, 서버, 네트워킹 장비 및 스토리지와 같은 물리적 인프라를 소유, 관리, 프로비저닝 및 유지하며 사용자는 가상화 된 컴퓨팅, 네트워킹 및 스토리지 리소스에 서비스로 액세스합니다.
...
: 사설 클라우드 모델에서 제공자는 단일 조직에서만 사용할 수 있도록 클라우드 인프라를 프로비저닝합니다. 사설 클라우드 인프라는 조직 내부에있을 수 있으며 실행 또는 온 프레미스 일 수 있습니다. 또는 Virtual Private Cloud (VPC)의 경우처럼 퍼블릭 클라우드에있을 수 있으며 클라우드 제공 업체가 소유, 관리 및 운영 할 수 있습니다.
...
: 조직의 온 프레미스 프라이빗 클라우드 및 타사 인 하이브리드 클라우드 모델에서 퍼블릭 클라우드는 퍼블릭 및 프라이빗 클라우드의 기능과 이점을 모두 활용하는 유연한 단일 인프라로 연결됩니다.
